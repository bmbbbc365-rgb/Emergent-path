...
## Confetti Reveal + Door Quote (Feb 2026)
- **Door quote**: The primary text on the `BlueprintDoor` face is now the user's specified quote — "You forged the key to break your chains; now cross the bridge to conquer your future. Step through—you earned this horizon." — italicized, `text-base leading-snug`. Applied to both locked and unlocked states. Same quote also appears in `JourneyStrip`, `WelcomeModal`, and the two Journey.jsx footer positions for consistency.
- **Confetti reveal**: `canvas-confetti` gold shimmer fires exactly once — the FIRST time a graduated participant lands on `/app/journey`. Persistence via new backend field `graduation_celebrated` on `journey_state` + new endpoint `POST /api/journey/graduation-celebrated` (200 no-op before graduation). Honors OS `prefers-reduced-motion` via `disableForReducedMotion: true`. Three staggered bursts (center + left + right) with gold palette (`#F5D28F #D4AF37 #F3E1D8 #EED2E0 #FBF3E9`).

## Batch B — Category Deep Pages (Feb 2026)
- **Backend**: 30 new substantive `kind=learn` resources added to `SEED_RESOURCES` in `resources_v2.py` across 6 categories — Emergency (FEMA kit, choking, AHA CPR, financial prep, CDC health), Financial (CFPB Money Goals, MyMoney.gov, AnnualCreditReport, Fed Reserve credit, HUD counselors), Health (CDC vaccines, Medicare, diabetes/BP self-care, MHA screening), Recovery (SAMHSA Recovery, NIDA Science, NAMI dual-diagnosis, Faces & Voices, Recovery Answers), Career (My Next Move, O*NET, CareerOneStop reentry, Indeed guide, DOL REO), Technology (DigitalLearn, Northstar, Google Digital Garage, Microsoft, Khan Academy). Seed now 75 rows.
- **Frontend**: new `pages/CategoryHub.jsx` at `/app/resources/category/:category` — warm-gradient hero (per-category accent), "Find local support in Arkansas" + `tel:211` CTAs, `Learn` grid + `Find Support` grid, category-scoped disclaimers footer. `Resources.jsx` hub now includes a "Deep dives · Learn + Find Support" strip with all 6 category cards linking directly into these pages.
